...
(async () => {
  ...
  try {
    ...
    requests[0].changes[0].edits = [{category:'invited_candidates',kind:'removed',before:{fullName:'Петров Пётр Сергеевич',position:''}}];
    ...
    requests[0].changes[0].edits = [{category:'invited_candidates',kind:'changed',before:{fullName:'Петров Пётр Сергеевич',position:''},after:{fullName:'Петров Пётр Александрович',position:''}}];
