...
(async () => {
  ...
  try {
    ...
    await page.route('**/*', async route => {
      ...
      else if (pathname === '/api/trainee-correction-requests') body = JSON.stringify([{ id: 'demo-trainee-request', reportDate: '2026-09-10', department: 'Алматы', reporterId: 1001, debtsterTraineeId: 2001, debtsterFullName: 'Иванов Иван Иванович', debtsterSource: 'ДИР', localRecord: { fullName: 'Иванов Иван Иванович', category: 'interns', reportDate: '2026-09-10', department: 'Алматы', responsible: 'Садыкова Айгуль', addedAt: new Date().toISOString(), reportRowId: row.id }, proposedChange: { source: 'ОК' }, note: 'Исправить источник стажёра.', status: 'pending', debtsterNote: '', requestedBy: user.id, requestedByName: 'Садыкова Айгуль', requestedByUsername: user.username, createdAt: new Date().toISOString() }]);
      else body = '[]';
    ...
    await shot('07-request-tabs-final');
    await page.locator('[data-employee-request-view="trainee"]').click();
    await shot('08-trainee-tab-final');
    await page.setViewportSize({ width: 520, height: 950 });
    await shot('09-trainee-tab-mobile-final');
    ...
    user.lastName = '';
    await page.reload();
    await page.locator('[data-request-view="trainee"]').click();
    await shot('10-trainee-admin-final');
    await page.locator('[data-request-view="employee"]').click();
